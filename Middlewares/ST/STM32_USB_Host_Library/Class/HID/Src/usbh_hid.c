...
/*
 * @brief  USBH_Get_Protocol

 */
USBH_StatusTypeDef USBH_HID_GetProtocol(USBH_HandleTypeDef *phost,
		uint8_t interface) {
	...
	memset(phost->device.Data, 0, 512);

	return USBH_CtlReq(phost, phost->device.Data, 1U);
